  ...
  drop(props, monitor, component) {
    ...
    if (isPaletteItem == true) {
        ...
        if (Math.round(listElementLeft + delta.x - paletteContainerWidth) < 0 && Math.round(item.top + delta.y) > (dropContainerElement.offsetHeight - 50)) {
          ...
          top = 0;
        } // prevent Devices leaving top left, top or left bounds
     ...
     */
        ...
        // const left = Math.min(Math.round(item.left + delta.x), dropContainerElement.offsetWidth - dropElement.offsetWidth);    // LEFT is geo:long
        // const top  = Math.min(Math.round(item.top + delta.y), dropContainerElement.offsetHeight - dropElement.offsetHeight);     // TOP is geo:lat
        ...
        {devices.map(storedDevice => {
          ...
          if (storedDevice["geo:location"]) {
            ...
            if (storedDevice["iot-lite:isSubSystemOf"] != null && storedDevice["iot-lite:isSubSystemOf"]["@id"] != "") {
              ...
              let targetOffSetX = 25;
              let targetOffSetY = 25;
          ...
          // <line x1={tempLeft + sourceOffSetX} y1={tempTop + sourceOffSetY} x2={targetLeft + targetOffSetX} y2={targetTop + targetOffSetY} style={{stroke: '#000000', strokeWidth: 2}} markerEnd={'url(#markerArrow)'} />
          ...
          /*
            Render Devices inside DropContainer
           */
