...
export default class ComponentDetailList extends React.Component {
    ...
    render() {
      ...
      const actionsSetProperty = [
        ...
        <FlatButton label="Save" primary={true} onTouchTap={ () => {DropActions.setProperty(this.state.id, [this.state.selectAttribute], this.state.textValue, this.state.key);
                ...
                <List style={styles}>
                  ...
                  {Object.keys(selectedDevice).map(key => {
                    ...
                    // primitive data as property value
                      ...
                      if (typeof  selectedDevice[key] === "string")
                        return (<ListItem onDoubleClick={ () => {
                                  if (key != "geo:location") {
                                    const tempDevice = utils.getObjectFromGraphById(selectedDevice["@id"], this.state.devices);
                                    this.setState({id: tempDevice["@id"], type: tempDevice["@type"], selectAttribute: key});
                ...
                >
                  {[1].map(
                    () => {
                      if (utils.isPrimitiveProperty(this.state.selectAttribute))
                        return (
                          <TextField value={this.state.textValue} onChange={ (e) => {this.setState({textValue: e.target.value}) } } hintText="New Value" />
                        );
                      else
                        return (
                          <SelectField
