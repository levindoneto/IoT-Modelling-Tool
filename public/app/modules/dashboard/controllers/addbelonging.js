dashboard.controller(
    'addbelongingController', [
        ...
        function (
            ...
            vm.addbelonging = function (prefix, type, model, file, pins) {
                // prefix->type->model
                ...
                Upload.base64DataUrl(file).then(base64Url => {
                    ...
                    imageList.$loaded().then(() => {
                        ...
                        imageList.$add(base64Url).then(imref => {
                            ...
                            refDevComp.on('value', snapshot => {
                                // The whole object savedModels with all the saved models
                                if (snapshot.val() != null) {
                                    if (prefix in snapshot.val() && add === false) {
                                        if (type in snapshot.val()[prefix]) {
                                            auxInfo[
                                                concatenate(
                                                    Object.keys(snapshot.val()[prefix][type]).length.toString(),
                                                    '-',
                                                    model.id
                                                )
                                            ] = model;
                                            ...
                                            const keyPrevious = Object.keys(snapshot.val()[prefix][type])[
                                                (Object.keys(snapshot.val()[prefix][type]).length - 1).toString()
                                            ];
                                            if (
                                                add === false &&
                                                snapshot.val()[prefix][type][keyPrevious].id !== model.id
                                            ) {
                                        ...
                                        // If prefix has not been defined
                                        // Create prefix
