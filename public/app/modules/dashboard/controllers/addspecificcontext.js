...
dashboard.controller('addspecificcontextController', ['$rootScope', '$scope', '$state', '$location', 'dashboardService', 'Flash', '$firebaseObject', '$firebaseArray',
function ($rootScope, $scope, $state, $location, dashboardService, Flash, $firebaseObject, $firebaseArray) {
    ...
    $scope.addspecificproperty = function (contextKey, newProperty, newPropertyValue) {
        const refSpecificContext = firebase.database().ref(`contexts/${contextKey}`); // Access context->contextKey on the database
    ...
    /* Function that emulates the (for i in range) with AngularJS 
     * for (min, max, step) {
     *     do something;
     * }
     */
