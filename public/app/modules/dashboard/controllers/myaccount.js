dashboard.controller(
    'myaccountController', [
        ...
        function (
            ...
            setTimeout(() => {
                try {
                    $scope.currentDefaultContext =
                        localStorage.getItem(DEFAULT_CONTEXT_ID) ||
                        allContexts[contextDefaultObj.$value].idcontext;
                } catch (err) {
                    console.log(err);
                }
            }, 2000);
            ...
            setTimeout(() => {
                try {
                    $scope.currentDefaultGraph =
                        localStorage.getItem(DEFAULT_GRAPH_ID) ||
                        allGraphs[graphDefaultObj.$value].idgraph;
                } catch (err) {
                    console.log(err);
                }
            }, 2000);
            ...
            $scope.verifySettingDefaultContext = function () {
                if (!contextDefaultObj.$value) {
                    // Default @context isn't set
            ...
            $scope.verifySettingDefaultGraph = function () {
                if (!graphDefaultObj.$value) {
                    // Default @graph isn't set
