        ...
        function (
            ...
            $scope.modal = function (keySelGraph) {
                if (!keySelGraph) {
                    ...
                    graphObj.$loaded().then(() => {
                        //Load the graphs from the database as an object
            ...
            $scope.setGraphDefault = function (keyGraph) {
                // The key is given by the user via a option box
                if (!keyGraph) {
                    swal({
                    ...
                    const refDefaults = firebase
                        .database()
                        .ref(
                            'defaults/'
                        );
                    /* defaults->defaultgraph provides the key for
            ...
            $scope.downloadGraphElement = function (keySelGraph) {
                // key is given by the user via a option box
                if (!keySelGraph) {
                    swal({
                    ...
                    const refGraphElement = firebase
                        .database()
                        .ref(`graphs/${keySelGraph}`);
                    ...
                    refGraphElement.once('value', snapGraph => {
                        ...
                        for (i in JSON.parse(snapGraph.val().defaultobjectsgraph)['@graph']) {
                            graphListAux.push(
                                JSON.parse(snapGraph.val().defaultobjectsgraph)['@graph'][i]
                            );
                        }
