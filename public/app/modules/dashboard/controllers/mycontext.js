            ...
            */
            function verifyAdditionalPropertyContext(elementPropertyI) {
                ...
                for (let prop = 0; prop < defaultContextProps.length; prop++) {
                    if (
                        ...
                        defaultContextProps[prop].toUpperCase()
                    ) {
            ...
            $scope.modal = function (keySelContext) {
                if (!keySelContext) {
                    ...
                    const refSelectedContext = firebase
                        .database()
                        .ref(`contexts/${keySelContext}`);
                    const contextObj = $firebaseObject(refSelectedContext);
                    contextObj.$loaded().then(() => {
                        //Load contexts from the database as an object
            ...
            $scope.downloadContextElement = function (keySelContext) {
                if (!keySelContext) {
                    ...
                    const refContextElement = firebase
                        .database()
                        .ref(`contexts/${keySelContext}`);
            ...
            $scope.getNestedElementsContext = function (elementAdditionalContext) {
                ...
                for (infoContextKey in elementAdditionalContext) {
                    if (elementAdditionalContext.hasOwnProperty(infoContextKey)) {
                        additionalContextInfo[infoContextKey] =
                            elementAdditionalContext[infoContextKey];
                    }
                }
            ...
            $scope.getAdditionalProperties = function (keySelContext) {
                const refKeySelContext = firebase
                    .database()
                    .ref(`contexts/${keySelContext}`); // Accesing the object context selected by the user
                const contextObj = $firebaseObject(refKeySelContext);
                const objAddPropsContext = {};
                /* Object with the key:value of the additional properties.
                * It'll be accessed via scope variable on the view */
                /* This is needed because of the asynchronous way of processing data */
                setTimeout(() => {
                    for (const contextPropI in contextObj) {
                        // Range on the object @context->key
                        if (contextObj.hasOwnProperty(contextPropI)) {
                            // This checks all properties' names on database's key
                            const isAddContProperty = verifyAdditionalPropertyContext(
                                contextPropI
                            );
                            if (isAddContProperty === true) {
                    ...
                    /* Now the object with the additional properties
