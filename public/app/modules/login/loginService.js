login.service('loginService', [
    ...
    'Flash',
    'apiService',
    function ($http, $q, Flash, apiService) {
        var loginService = {};

        //service to communicate with users model to verify login credentials
        var accessLogin = function (parameters) {
            var deferred = $q.defer();
            apiService.get('users', parameters).then(
                function (response) {
                    if (response) deferred.resolve(response);
                    else
                        deferred.reject(
                            'Something went wrong while processing your request. Please Contact Administrator.'
                        );
                },
                function (response) {
        ...
        var registerUser = function (parameters) {
            var deferred = $q.defer();
            apiService.create('users', parameters).then(
                function (response) {
                    if (response) deferred.resolve(response);
                    else
                        deferred.reject(
